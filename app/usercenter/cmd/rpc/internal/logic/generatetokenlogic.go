...
func (l *GenerateTokenLogic) GenerateToken(in *userpb.GenerateTokenReq) (*userpb.GenerateTokenResp, error) {
	now := time.Now().Unix()
	accessExpire := int64(l.svcCtx.Config.JwtAuth.AccessExpire)
	accessToken, err := l.getJwtToken(l.svcCtx.Config.JwtAuth.AccessSecret, now, accessExpire, in.UserId)
	if err != nil {
		return nil, err
	}

	return &userpb.GenerateTokenResp{
