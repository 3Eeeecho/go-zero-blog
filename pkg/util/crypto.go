...
func EncryptPassword(c config.Config, plainPassword string) (string, error) {
	key := []byte(c.Crypto.Key)
	if len(key) != 32 {
...
func DecryptPassword(c config.Config, encryptedPassword string) (string, error) {
	key := []byte(c.Crypto.Key)
	if len(key) != 32 {
		return "", errors.New("AES key must be 32 bytes")
	}
